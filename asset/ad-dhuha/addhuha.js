  ...
  container.innerHTML = `
    <div class="surah-tool-container ${currentState.darkMode ? 'dark-mode' : ''}">
      <div class="tool-header">
        <h3>🌅 Tool Interaktif Surah Ad-Dhuha</h3>
        <p>Bacaan audio, progress tracking &amp; bookmark-semua dalam satu platform</p>
      </div>
      ...
      <div class="controls-panel">
        ...
        <button class="control-btn ${currentState.darkMode ? 'active' : ''}" onclick="toggleDarkMode()">
          ${currentState.darkMode ? '☀️' : '🌙'} ${currentState.darkMode ? 'Light' : 'Dark'}
        </button>
...
function renderVerses() {
  ...
  container.innerHTML = surahData.verses.map((verse, index) => `
    <div class="verse-card ${currentState.currentVerse === index && currentState.isPlaying ? 'playing' : ''}" id="verse-${index}">
      <div class="verse-header">
        <div class="verse-number">${verse.number}</div>
        <div class="verse-actions">
          <div class="action-icon ${currentState.progress[index] ? 'active' : ''}" 
               onclick="toggleProgress(${index})" 
               title="Tandakan sebagai dikuasai">
            ✓
          </div>
          <div class="action-icon ${currentState.bookmarks.includes(index) ? 'active' : ''}" 
               onclick="toggleBookmark(${index})" 
               title="Bookmark ayat">
            ⭐
          </div>
          <div class="action-icon" onclick="playVerse(${index})" title="Mainkan audio">
            🔊
          </div>
        </div>
      </div>
      ${currentState.showArabic ? `<div class="verse-arabic">${verse.arabic}</div>` : ''}
      ${currentState.showRumi ? `<div class="verse-rumi">${verse.rumi}</div>` : ''}
      ${currentState.showMalay ? `<div class="verse-translation">${verse.translation}</div>` : ''}
    </div>
  `).join('');
}
...
function toggleProgress(index) {
  ...
  localStorage.setItem('surahAdDhuhaProgress', JSON.stringify(currentState.progress));
  updateStats();
  renderVerses();
}
...
function toggleBookmark(index) {
  ...
  localStorage.setItem('surahAdDhuhaBookmarks', JSON.stringify(currentState.bookmarks));
  updateStats();
  renderVerses();
}

function resetProgress() {
  if (confirm('Adakah anda pasti mahu reset semua progress?')) {
...
function updateStats() {
  const completed = Object.values(currentState.progress).filter(v => v).length;
  const percentage = Math.round((completed / surahData.verses.length) * 100);
  ...
  if (bookmarkElement) bookmarkElement.textContent = currentState.bookmarks.length;
}

// Share Functions
function shareToWhatsApp() {
  const url = encodeURIComponent(window.location.href);
  const text = encodeURIComponent('Baca Surah Ad-Dhuha dengan tool interaktif yang awesome! Ada audio, rumi, tafsir, dan progress tracking. Cuba sekarang:');
...
function copyLink() {
  navigator.clipboard.writeText(window.location.href).then(() => {
    alert('Link berjaya disalin! Kongsi dengan kawan-kawan anda.');
  });
}
