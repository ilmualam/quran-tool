...
(function() {
    ...
    const CONFIG = {
        ...
        storageKey: 'ilm_muzzammil_bookmarks',
        colors: {
            primary: '#1a237e',
            secondary: '#0d47a1',
            text: '#0c3808'
    ...
    const _0x7f3c=['SW1sdUFsYW0uY29t','QWxsIHJpZ2h0cyByZXNlcnZlZA=='];
    ...
    function injectStyles() {
        if (document.getElementById('ilm-muz-styles')) return;
        
        const styles = `
        .ilm-x-reader{max-width:900px;margin:32px auto;padding:16px;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;background:#fff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.1)}
        .ilm-x-header{background:linear-gradient(135deg,${CONFIG.colors.primary} 0%,${CONFIG.colors.secondary} 100%);color:#fff;padding:20px;border-radius:8px 8px 0 0;margin:-16px -16px 24px;text-align:center}
        ...
        .ilm-x-btn-primary:hover{background:${CONFIG.colors.secondary};transform:translateY(-2px)}
        ...
        .ilm-x-ayah.ilm-x-playing{background:#e8eaf6;border-color:${CONFIG.colors.primary}}
        ...
        .ilm-x-arabic{font-size:1.8rem;line-height:2.5;text-align:right;direction:rtl;font-family:"Traditional Arabic","Scheherazade",serif;color:${CONFIG.colors.text};margin:16px 0}
        ...
        container.innerHTML = `
            <div class="ilm-x-reader">
                <div class="ilm-x-header">
                    ...
                    <div class="ilm-x-meta">${CONFIG.totalAyahs} Ayat | Makkiyyah</div>
                </div>
    ...
    function render(filteredAyahs = null) {
        ...
        const html = `
            <div class="ilm-x-reader">
                <div class="ilm-x-header">
                    <h2>Surah ${CONFIG.surahName} (المزمل)</h2>
                    <div class="ilm-x-meta">🌙 ${CONFIG.totalAyahs} Ayat | Surah Makkiyyah | Juzuk 29 | Panduan Solat Tahajjud</div>
                </div>
                ...
                <div class="ilm-x-controls">
                    <input 
                        ...
                        id="ilm-search-input"
                    >
                    <button class="ilm-x-btn ilm-x-btn-secondary" onclick="ilmMuzReader.showBookmarks()">
                        ⭐ Simpanan (${state.bookmarks.length})
                    </button>
                    <button class="ilm-x-btn ilm-x-btn-secondary" onclick="ilmMuzReader.stopAllAudio()">
                        ⏸️ Hentikan Audio
                    </button>
                </div>
                ...
                ${filteredAyahs ? `
                    <div class="ilm-x-filter-info">
                        ...
                        <button class="ilm-x-btn ilm-x-btn-secondary" style="margin-left:12px;padding:4px 12px" onclick="ilmMuzReader.clearSearch()">
                            ✕ Padam Carian
                        </button>
                    </div>
                ` : ''}
        ...
        return `
            <div class="ilm-x-ayah ${isPlaying ? 'ilm-x-playing' : ''} ${isBookmarked ? 'ilm-x-bookmarked' : ''}" id="ayah-${ayah.numberInSurah}">
                ...
                <div class="ilm-x-ayah-actions">
                    <button class="ilm-x-action-btn ${isPlaying ? 'ilm-x-active' : ''}" onclick="ilmMuzReader.playAudio(${ayah.numberInSurah}, '${ayah.audio}')">
                        ${isPlaying ? '⏸️ Henti' : '▶️ Dengar'}
                    </button>
                    <button class="ilm-x-action-btn" onclick="ilmMuzReader.copyAyah(${ayah.numberInSurah})">
                        📋 Salin
                    </button>
                    <button class="ilm-x-action-btn" onclick="ilmMuzReader.shareAyah(${ayah.numberInSurah})">
                        🔗 Kongsi
                    </button>
                    <button class="ilm-x-action-btn ${isBookmarked ? 'ilm-x-active' : ''}" onclick="ilmMuzReader.toggleBookmark(${ayah.numberInSurah})">
                        ${isBookmarked ? '★' : '☆'} Simpan
                    </button>
                </div>
            </div>
...
Sumber: IlmuAlam.com`;
    ...
    // Console signature
    console.log('%c' + atob(_0x7f3c[0]), 'color:#1a237e;font-weight:bold;font-size:14px');
    console.log('%c' + atob(_0x7f3c[1]), 'color:#666;font-size:12px');
